...
- **Roulette** — 🎲 pour lancer autre chose que de la ranked sans débattre dix minutes. Deux tirages : **Compo d'équipe** et **Un agent** (au hasard, filtrable par rôle).
  ...
  Trois façons de définir la compo : **Équilibrée** (un rôle différent par joueur), **Libre** (n'importe quel agent), et **Sur mesure** — tu dis combien de chaque rôle, par exemple *2 initiateurs + 1 contrôleur + 2 duellistes*. Les places non attribuées restent **libres**. Si tu demandes plus de rôles que de joueurs, le surplus est ignoré et c'est dit.

  **L'animation** : toutes les roues tournent en même temps, puis se verrouillent **une par une** avec un flash — on voit la compo se figer place par place, et il reste toujours quelque chose qui tourne jusqu'au dernier (~4 s, 4 roues encore en mouvement à 1,8 s). `prefers-reduced-motion` donne le résultat direct.
