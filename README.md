...
- **Affichage instantané (cache local) + voyant de fraîcheur** — l'app peint l'écran **avant** que l'API réponde, à partir de ce qu'elle savait la dernière fois, puis rafraîchit **systématiquement** en arrière-plan : le cache n'évite jamais un appel, il évite l'écran d'attente. Mesuré en navigateur (API bridée à 1,5 s) : **rangs affichés en 95 ms**, **profil complet en 58 ms**.

  Ce qui est stocké est une projection **mince** — sur 20 parties : 4 134 Ko bruts de l'API, 972 Ko une fois normalisés, **8,9 Ko** en projection. Stocker le normalisé ferait exploser le quota (~5 Mo) dès le 5ᵉ membre ; le détail par round et le scoreboard complet restent donc dehors, de toute façon rechargeables au clic. Le détail du calcul de l'indice contient des fonctions (non sérialisables) : on stocke le **contexte** et on le recalcule à la relecture, donc la modale s'ouvre aussi sur une partie venue du cache. Le cache porte une **version de schéma** : écrit par une version antérieure, il est jeté plutôt que relu de travers. Quota dépassé → éviction des profils les plus anciens ; stockage indisponible (navigation privée) → l'app tourne sans instantané.

  Le **voyant** (en haut à droite de l'accueil et du profil) a quatre états, dont **un seul est actionnable** : 🟢 à jour · 🟡 mise à jour en cours · 🟠 affiché depuis le cache · 🔴 **le dernier rafraîchissement a échoué** (avec la raison, et un clic pour réessayer). Il affiche l'**âge** en toutes lettres (« il y a 3 min »), qui vieillit tout seul — le tic s'arrête quand l'onglet passe en arrière-plan.

  Conséquence la plus utile : **une erreur ne vide plus la page**. En cas de 429 pendant le rafraîchissement, les données restent à l'écran et seul le voyant passe au rouge.
- **Résistance au rate limit** — les appels HenrikDev partent par **paquets de 3** au lieu d'une rafale (les rangs de l'accueil en lançaient 8 d'un coup, c'était la cause des 429), avec **retry et backoff** sur 429/5xx en écoutant l'en-tête `Retry-After` que le proxy relaie désormais. Les erreurs sont dites en français (« trop de requêtes d'un coup côté API Valorant ») au lieu d'un `http 429` brut. La sauvegarde d'historique déclenchée à l'ouverture d'un profil est **différée de 5 s** pour ne pas s'ajouter à la rafale.
- **Profil joueur** — rang actuel (icône + RR), indice COSMO /100, liste des matchs filtrable par mode. Chaque partie classée affiche le **RR gagné/perdu** et le **rang du joueur au moment de la game** ; un clic ouvre sa modale complète. Pour les parties venues du blob (format compact), le scoreboard complet est **chargé à la demande** via l'endpoint match-by-id, et la modale se met à jour toute seule quand il arrive.
...
- **Détail d'une partie (modale unique)** — **cliquer une partie de la liste ouvre tout d'un coup** : plus de scoreboard en bas de page ni de second bouton. La modale réunit un **en-tête** (indice cliquable, résultat, map, agent, durée, ±RR), le **scoreboard complet** (tête d'agent, classement ACS, indice de chaque joueur), un **déroulé** (mi-temps par mi-temps, prolongations, meilleure série de rounds, rounds joués), puis : **timeline round par round** (gagné/perdu, comment le round s'est fini, tes kills, spike posée/désamorcée, badges Ace/Clutch/Flawless/Thrifty) avec le **feed des éliminations** de chaque round au clic ; **faits d'armes** (first bloods/deaths, multikills, clutches avec leur taille, spikes) ; **armes & précision** (kills par arme, répartition exacte tête/corps/jambes) ; **économie & utilitaire** (achat moyen, crédits dépensés par round, répartition eco / demi-achat / full buy **avec le taux de victoire de chaque tranche**, et compétences utilisées dont les ultimates) ; **duels** (dégâts infligés/subis face à chaque adversaire) ; et **lobby** (rang de chacun + groupes détectés via `party_id`). Cliquer l'indice d'un joueur ouvre le **détail du calcul par-dessus**, sans fermer la partie ; `Échap` ne ferme que la modale du dessus. *(Le HS% **par arme** n'est pas exposé par l'API — aucun indicateur de headshot sur un kill ni sur un évènement de dégâts — il n'est donc pas affiché plutôt qu'approximé.)* Tout vient des données de round déjà téléchargées avec la partie — **aucun appel API supplémentaire** — et n'est conservé que sous forme compacte (~28 Ko au lieu des ~460 Ko bruts).
...
- **Invités (hors squad)** — ⚙ Paramètres → *Modifier le roster* → section **Invités** : des gens avec qui vous jouez parfois sans qu'ils soient de la squad. Ils apparaissent dans la **composition** (« Trio · SevenDayy, Kevin\* ») et dans le **rapport commun** des sessions, marqués *(invité)*, et **nulle part ailleurs** : ni carte d'accueil, ni leaderboard, ni tribunal, ni alertes, ni rafraîchissement automatique. Comme le cron ne les visite pas, ils n'ont en général pas d'historique stocké — leurs chiffres viennent alors **du scoreboard de la partie**, ce qui suffit pour tout le récent. Un invité déjà membre de la squad est ignoré (pas de doublon), et il peut avoir des anciens pseudos comme un membre.
...
- **Alertes de session (accueil)** — un bandeau discret quand une session récente mérite qu'on en parle : *« Yakuza — 7 parties d'affilée hier soir, les 3 dernières bien en dessous (indice 77 → 50). »*. Trois déclencheurs : **session trop longue qui s'effondre** (≥ 6 parties et baisse d'indice entre les deux moitiés), **grosse chute de RR**, et **belle remontée** — pour que le bandeau ne soit pas qu'un rabat-joie.

  Sélection : **une seule alerte, celle de la session la plus récente**, tous membres confondus, et **rien au-delà de 48 h**. Sur cette session, c'est le constat le plus marquant qui sort. Un bandeau qui empile plusieurs évènements de plusieurs jours devient un mur qu'on ne lit plus. Un clic ouvre le rapport de la session.

  Les dates sont dites en **journées de jeu**, qui **coupent à 5 h du matin, pas à minuit** (`DAY_CUTOFF_H`) : une partie jouée dimanche à 2 h appartient à la soirée du **samedi**. Sans ça, à 2 h du matin le bandeau annoncerait « hier soir » une session finie il y a une heure — il dit « ce soir ».
...
- **Roulette** — 🎲 quand on veut lancer autre chose que de la ranked et laisser le destin choisir. Trois tirages : **Compo** (qui joue *et* avec quel agent, en solo / duo / trio / quatuor / 5-stack), **Agent** (un agent au hasard, filtrable par rôle), **Personnes** (qui joue). L'animation ralentit puis **atterrit pile sur le gagnant** — le nombre de crans est calculé pour que la dernière image affichée soit bien le résultat, sinon l'animation et le tirage se contrediraient. Les révélations s'enchaînent une par une pour le suspense, et `prefers-reduced-motion` donne le résultat direct.

  Deux options sur la compo : **⚖ équilibrée** (un rôle différent par joueur tant qu'il en reste — au 5ᵉ, les quatre rôles sont pris, il est donc libre) et **🆕 persos à tester**, qui privilégie pour *chaque personne* les agents qu'elle joue le moins d'après l'historique déjà chargé. Quand tout a déjà été joué, on garde les **moins** joués plutôt que de ne rien rendre. Deux joueurs n'ont jamais le même agent ; s'il n'y en a pas assez dans le rôle demandé, on autorise le doublon plutôt que de laisser quelqu'un sans agent. Les **invités** participent aux tirages.
