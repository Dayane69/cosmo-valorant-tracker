...
- **Sessions solo / duo / trio + rapport commun** — la composition est **ancrée sur le roster courant**, à partir de deux sources complémentaires :
  ...
  > **Changement de pseudo et historique.** Le blob d'historique est indexé par `pseudo#tag` : renommer un membre laisse son ancien blob orphelin et en repart d'un vide. Les parties récentes continuent de marcher (elles viennent de `matches` v4), et l'historique se **reconstruit tout seul** au prochain passage du cron — ou tout de suite via ⚙ Paramètres → *« ⟳ Sauvegarder l'historique de la squad »*, puisque `stored-matches` est rattaché au compte et non au pseudo.
